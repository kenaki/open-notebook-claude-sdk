...
>
> **Update (2026-07-03, executor live-verification pass):** Drove the parked non-visual live checks
> against the running services + the one real PDF (`source:au8…`, 286 embeddings / **285 page_number** /
> 121 distinct pages / 1 whole-doc section / abstract+summary present). Results:
> • **B5 → ☑** — one real Claude-agent turn (default chat model = `claude_agent`/`claude-opus-4-8`) called
>   **both** `get_source_outline` + `get_section` and answered the chapter question; 4000-char cap confirmed.
> • **Phase3 `#p=N` emission → ✅ VERIFIED** (local `qwen3.6:35b` emitted 6 well-formed page citations via the
>   real `ask` path). Phase3 **stays ◐** for (a) the visual click-lands-on-page smoke and (b) a newly-surfaced
>   **page-accuracy caveat** (`fn::vector_search` per-source grouping keeps only the first chunk's page ⇒ cited
>   page can be wrong — see Open Questions `X-page-accuracy`).
> • **B2 stays ◐** — render/skip-guard/split verified non-destructively, but the vision "cleaned > raw" run is
>   **data-blocked**: the lone PDF is a 185-page single section > the 50-page cap → skip path. Needs a PDF that
>   chapters into ≤50-page sections.
...
| B | B2 | Per-chapter verify-clean background command | ◐ | commit 09c0fed (wave5b 2026-07-02); `verify_clean_section`+`verify_clean_source` cmds, PyMuPDF 2× render, cleaned→`cleaned_content` (raw immutable), discrepancies→`verify_flag` insight, fire-and-forget trigger in `submit_sections`. Static verify green (imports+register, pytest 31). **LIVE partial ✅ 2026-07-03 (executor, non-destructive):** PyMuPDF `_render_section_pages` renders the real PDF (2 PNGs ~205–211 KB for pp.51–52); skip-guard math confirmed (span 185 > `_MAX_VERIFY_PAGES=50`); `_split_cleaned_and_discrepancies` correct (split + none-collapse). **⛔ QUALITY RUN STILL PARKED (data-blocked, NOT code):** the only source is one 185-page whole-doc section (no TOC → Decision #4 fallback) > 50-cap → `verify_clean_section` takes the **skip-with-verify_flag** path, so the vision "cleaned > raw" deliverable is unexercised. Needs a PDF that chapters into ≤50-page sections (bookmarked/multi-chapter or short PDF) to run the real vision clean. Auto-decisions: direct `get_vision_model(max_tokens=8192)` (NOT `provision_langchain_model`); retry `max_attempts=3/fixed 10s/stop_on ValueError,ConfigError`; `_MAX_VERIFY_PAGES=50` skip-with-flag. **🏁 ARCHIVED 2026-07-06 with this ◐ open — handed to the user as an individual follow-up (data-blocked, not a code defect).** |
...
| B | B5 | Agent tools get_source_outline / get_section | ☑ | commit 1effece (wave5d 2026-07-02); `get_source_outline`+`get_section` @tool (conformed to file's real dict-arg/`_result` pattern), registered in MCP server; `get_source` kept full_text + added nav hints; `get_section` content capped at 4000 chars. **LIVE ✅ VERIFIED 2026-07-03 (executor):** default chat model IS the claude_agent (`claude-opus-4-8`, `is_claude_agent_selected=True`); drove one real agent turn on `source:au8…` — the agent called **both** `get_source_outline` AND `get_section` (section id taken from the outline) + `get_source`/`search`, answered the chapter question ("11 chapters, Ch4 = present tense", cited section id), no crash → all 5 Verify criteria met. 4000-char cap confirmed live (`get_section` content_len=4013 ending `… [truncated]`). ⚠ Data note: this PDF is one whole-doc section (no TOC), so `get_section` returns truncated front-matter not a per-chapter body — richness gap tied to single-section test data, not a B5 defect |
...
| 20 | citation → PDF-page-open (Q-citation-pdf-open)? | **PARKED FOLLOW-UP (from C4):** Phase3 forwards a `page` arg and C4 does section-anchor scroll, but opening the inline PDF at page N needs `SourceDetailContent.tsx` made tab-controlled + `PDFViewer.initialPage` wired + a `{activeTab, citationPage}` lift on the source page — all OUTSIDE any chunk's file ownership (Phase1 owns those, done). **Proposed default:** a small follow-up chunk (allowed to touch SourceDetailContent + source page + PDFViewer) closes it; `ChatPanel.handleReferenceClick` then takes the 3rd `page` arg. On the run's end punch-list. **✅ IMPLEMENTED 2026-07-02 (commit ed82b2e)** — modal-based (uniform across every chat context, no source-page tab lift needed): `openModal(type,id,page?)` carries the page as a URL param → `SourceDialog`/`SourceDetailContent` (controlled tabs) jump to the PDF tab and pass `initialPage` (1-indexed citation → 0-based viewer) to `PDFViewer` (keyed to re-navigate); `handleReferenceClick` takes the 3rd `page`. `npm run build` ✅. Visual click-through = manual check. |
...
- ~~**X-page-accuracy**~~ **✅ RESOLVED (migration 21, 2026-07-03)** *(was emergent/needs-user, ~S2/S3)* — `#p=N`
  ...
  **single** (first) page number for a **multi-page** text blob; the LLM then attributes pages by guessing
  (observed: cited pp.52–56 when only p.62 was provided, and the sole returned source row had empty `content`
  with the chunk text folded into `matches`). The grouping was inherited from migration 4 and kept deliberately.
...
## Changelog
- 2026-07-06 (**ARCHIVAL — document-foundation closed out**) — all code chunks ☑ and committed;
  `to-fix/003` chaptering fix landed (`9ec7b8a`) + re-chapter verified live. Remaining ◐ chunks **handed to
  the user as individual follow-ups** per the cross-plan finish instruction: **B2** (data-blocked — needs a
  PDF that chapters into ≤50-page sections to exercise the vision "cleaned > raw" quality run; code landed +
  static-verified + non-destructive live-partial verified) and **Phase3** (visual click-lands-on-page browser
  smoke; all code + live data-path already verified, incl. X-page-accuracy via mig 21). The `003`/`b2-b3`
  full-book data-quality re-run validation is likewise the user's to finish. Final merged-tree verify green
  (backend 32 + baseline flake; `tsc` exit 0). Directory archived to `archived/document-foundation/`.
...
- 2026-07-03 (executor live-verification pass — no code changes) — Drove the parked non-visual live checks
  ...
  (`source:au8…`, "Modern Greek Grammar Notes.pdf"). DB recon: 286 embeddings / **285 page_number-stamped** /
  286 section-stamped / 121 distinct physical pages / **1** whole-doc section (pp.0–184, no TOC) / abstract
  (764c) + section summary (1434c) present — confirms B3/B4/Phase3-data live. **B5 → ☑:** the default chat
  model is the `claude_agent` (`claude-opus-4-8`); one real agent turn called **both** `get_source_outline`
  and `get_section` (id from the outline) and answered "11 chapters, Ch4 = present tense" without crashing;
  4000-char cap confirmed (`get_section` content_len=4013 `… [truncated]`). **Phase3 `#p=N` emission ✅
  VERIFIED:** the real `ask`/`provide_answer` path (vector_search→`query_process.jinja`→`qwen3.6:35b`) produced
  6 well-formed `[source:au8…#p=N]` citations (needs `max_tokens≥8000` — qwen3.6 thinking model empties out at
  the ask default of 2000). **New caveat `X-page-accuracy` (needs-user):** `fn::vector_search` groups by source
  id and keeps only the first chunk's page for a merged multi-page blob → cited page can be wrong (cited pp.52–56
  vs provided p.62). **B2:** render/skip-guard/split verified non-destructively; the vision "cleaned > raw" run
  stays **data-blocked** (185-page single section > 50-cap → skip path). Phase3 + B2 remain ◐; df un-archived.
  Residual = user punch-list: visual smokes (Phase1/C3/C4/Phase3-click), decide `X-page-accuracy`, supply a
  ≤50-page-section PDF for B2.
- 2026-07-02 (executor resume, Decision #20 BUILT) — **citation `#p=N` → open inline PDF at page**
  (commit ed82b2e). Chose the **modal-based** approach (uniform across notebook dock / side / source-page
  chat; no per-source-page tab lift needed): `use-modal-manager.openModal(type,id,page?)` carries the page
  as a URL param and exposes `modalPage`; `ModalProvider → SourceDialog → SourceDetailContent` thread
  `initialPage`; `SourceDetailContent` uses controlled tabs, jumps to the PDF tab and passes `initialPage`
  (1-indexed citation page → 0-based viewer index) to `PDFViewer` (keyed to re-navigate on page change);
  `ChatPanel.handleReferenceClick` takes the 3rd `page` arg (already forwarded by the source-references
  parser) — page citations open the PDF, non-page citations keep the C4 card/section scroll. `npm run build`
  EXIT=0; `on-frontend` GET /notebooks 200. Visual click-through is the one manual check left. 5 files, no
  new i18n. Phase3's citation→PDF-open sub-item is now ✅ — only the live-chat `#p=N` emission confirmation
  remains ◐.
