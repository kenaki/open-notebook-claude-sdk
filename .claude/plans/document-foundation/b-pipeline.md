...
| B2 | Per-chapter verify-clean background command | ◐ | commit 09c0fed (wave5b 2026-07-02); `verify_clean_section`+`verify_clean_source` in `commands/verify_commands.py`, fire-and-forget trigger in `submit_sections`, `commands/__init__.py` wired. PyMuPDF 2× render → `get_vision_model(max_tokens=8192)` (direct, not provision — see coord Decisions #14) → `cleaned_content` (raw immutable) + `verify_flag` insight. Static verify green (import+register, pytest 31). **LIVE partial ✅ 2026-07-03 (non-destructive):** render works on the real PDF (2 PNGs ~205–211 KB pp.51–52); skip-guard math (185 > 50) + `_split_cleaned_and_discrepancies` confirmed. **⛔ QUALITY RUN DATA-BLOCKED:** lone PDF = 1×185-page section > 50-cap → skip-with-flag path, so vision "cleaned>raw" is unexercised; needs a PDF that chapters into ≤50-page sections. Auto-decisions #14–16 in coordinator |
| B3 | Per-section summaries + doc abstract | ☑ | commit abb9444 (wave5c 2026-07-02); summary_commands.py (summarize_section + generate_source_abstract), Source.summarize_sections() fan-out, trigger after B2 verify. Static green (register, full suite 214). **LIVE ✅ VERIFIED 2026-07-02** — 1434-char section summary + 764-char abstract, surfaced by get_context("long") (re-confirmed in DB 2026-07-03). Auto-decisions coord #17–19 |
| B4 | Tiered get_context rewrite (the digestion fix) | ☑ | commit c6f6004 + fix 8c0b606 (wave5d 2026-07-02); get_context("long")→ tiered digest for chaptered sources, **full_text fallback for non-chaptered (Decision #21 RESOLVED, #12 honored)**; Notebook.get_context rewired; 215 pass. **LIVE no-balloon ✅ VERIFIED 2026-07-02** (chaptered→{abstract,outline}, no full_text; 281,733→3,378 chars = 98.8%↓) |
| B5 | Agent tools get_source_outline / get_section | ☑ | commit 1effece (wave5d 2026-07-02); get_source_outline+get_section MCP tools (real dict-arg pattern), get_source nav hints, 4000-char section cap. 214 pass, ruff clean, MCP builds. **LIVE ✅ VERIFIED 2026-07-03 (executor):** default chat model = claude_agent (claude-opus-4-8); one real agent turn called both get_source_outline AND get_section (id from outline) + answered the chapter question, no crash → all 5 Verify criteria met; 4000-char cap confirmed live (content_len 4013 `… [truncated]`). ⚠ single-section test PDF → get_section returns truncated front-matter (data limit, not a defect) |
